...
class Train():
    ...
    def rotate(self,board):
        new = [[board.grid[0][2],board.grid[1][2],board.grid[2][2]],[board.grid[0][1],board.grid[1][1],board.grid[2][1]],[board.grid[0][0],board.grid[1][0],board.grid[2][0]]]
        board.grid = new

    def flip(self,board):
        new = [[board.grid[0][2],board.grid[0][1],board.grid[0][0]],[board.grid[1][2],board.grid[1][1],board.grid[1][0]],[board.grid[2][2],board.grid[2][1],board.grid[2][0]]]
        board.grid = new

    def symmetry(self,board,number):
        if number<5:
    ...
    def simulate(self, xbot, obot, print_game = False):
        for j in range(self.num_of_games):
            ...
            if j % 1000 == 0:
                ...
                if test.o_wins < 20:
                    xbot.savePolicy("/home/svu/e0235225/FYP/TicTacToe/REpo_Vmodel1_round{}".format(str(j)))
                # print(self.states)
            for i in range(9):
                choice = []
                self.symmetry(board,random.randint(1,8))
